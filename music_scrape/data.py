...
def _show_one(audio_file):
    ...
    print
    'Artist:      ', pytrack.artist if hasattr(pytrack, 'artist') else 'Unknown'
    print
    'Title:       ', pytrack.title if hasattr(pytrack, 'title') else 'Unknown'
    print
    'Track ID:    ', pytrack.id
    print
    'Tempo:       ', pytrack.tempo
    print
    'Energy:       %1.3f %s' % (pytrack.energy, _bar(pytrack.energy))
    ...
    print
    'Valence:      %1.3f %s' % (pytrack.valence, _bar(pytrack.valence))
    print
    'Acousticness: %1.3f %s' % (pytrack.acousticness, _bar(pytrack.acousticness))
    print
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print
        'usage: python show_tempos.py path'
    else:
